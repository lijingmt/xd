...
Public request work runs in a bounded `Thread.Farm`, but per-account transaction locks preserve causality. The default pending limit is 128; accepted work is rejected with `503` and `Retry-After: 1` when routing is paused or saturated. Do not make the queue unbounded.
...
- direct equipment/item gift is disabled in distributed mode;
- face-to-face trade is disabled in distributed mode;
