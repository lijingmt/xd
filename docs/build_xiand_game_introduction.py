...
def add_cover(
    ...
    cover_box = Table(
        [
            [
                Paragraph(
                    f"十二个职业 / {skill_count} 个职业技能对象 / {book_count} 条职业技能书配置<br/>"
    ...
    guide.md.extend(
        [
            ...
            f"- 内容基线：`{commit}`",
            f"- 生成日期：{build_date}",
            f"- 数据规模：12 个职业，{skill_count} 个职业技能对象，{book_count} 条职业技能书配置，15 种公开山海异兽 + 1 只隐藏鸾鸟",
            "",
            "> 本介绍依据当前 main 代码、配置与测试文档生成；活动、库存和运营数值以游戏内实时显示为准。",
            "",
        ]
...
def build_game_introduction() -> None:
    ...
    guide.table(
        ["核心支柱", "玩家获得的体验", "代表系统"],
        [
            ["职业差异", "坦克、治疗、召唤、法术、物理爆发和持续伤害都有独立循环", "十二职业、职业任务、隐藏技能"],
    ...
    guide.bullets(
        [
            ...
            "十二职业合计拥有34本极稀有隐藏大神技能书，只从实际等级70级以上怪物的资格池掉落，不在商店出售。",
    ...
    guide.table(
        ...
        [
            ...
            ["图鉴培养", "有效等级不超过当前人物等级，共享高等级进度会保留；十星、五阶羁绊；真实PVE战斗自动获得宠物经验", "收集、升级、升星、进化、异色和组合展示"],
            ...
            ["万灵裂隙", "3-5个不同角色协作，同账号多角色可补足行动位；12轮内完成破阵、守御、疗愈、封印和缚灵", "周轮替异兽、团队分工、共享账号每场一份奖励、完整灵卵与保底"],
    ...
    guide.table(
        ...
        [
            ...
            ["xiand-all-professions-skill-guide.pdf", "十二职业全部技能、隐藏技能、书籍来源与学习条件"],
