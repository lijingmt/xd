...
幻境区同样使用全部 Worker，不固定切成“永恒 4 个、幻境 4 个”。例如 S1 将公共
月门营地固定为一个 `hub` affinity，把三段相连野外章节划为 `silver`、`ruins`、
`depths`。因此大量玩家分散探索时可以自然使用多个进程；进入同一具体房间时仍会
汇聚到该 affinity 的唯一 owner。副本继续使用服务端队伍/实例键，同一队伍进入的
同一副本不会被拆开。这个物理分布不会改变 `illusion:S1` 的逻辑隔离规则。
