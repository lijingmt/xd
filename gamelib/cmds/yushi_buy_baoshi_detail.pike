...
int main(string|zero arg)
{
	...
	if(!err && yushi){
		...
		if(flag == 0){
			...
			int capacity=SHOP_BATCHD->query_capacity(me,yushi,0);
			...
			if(quick_max>0){
				foreach(({1,5,10,20,50,100}),int quick)
					if(quick<=quick_max)
						s += "[买"+quick+"个:yushi_buy_baoshi_confirm "+
							yushi_name+" "+rarelevel+" "+amount+" "+money+
							" 0 "+quick+"] ";
				if(search(({1,5,10,20,50,100}),quick_max)==-1)
					s += "[按余额/背包买满"+quick_max+"个:"+
						"yushi_buy_baoshi_confirm "+yushi_name+" "+
