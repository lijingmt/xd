		...
		if(sscanf(s,"[%s]",buf)){
			if(sizeof(buf)&&buf[0]=='<'){
			   ...
			   has_suffix(href,".png") && search(href,"..")==-1){
				...
				out+=sprintf("<div role='img' aria-label='新月长生劫剧情插画' style=\"width:100%%;max-width:544px;aspect-ratio:1/1;margin:12px auto;background-image:url('%s');background-size:300%% 300%%;background-position:%d%% %d%%;background-repeat:no-repeat;border:2px solid #caaeef;border-radius:16px\"></div>",
