...
{
	test_start("二级与强化技能书真实学习");
	object|zero player = 0;
	object|zero book = 0;
	object|zero original_player = this_player();
	mapping(string:string) enhanced_books = ([
		"lingbailei11":"lingbailei11",
		"lingxuanying2":"lingxuanying2",
		"sanlingheyi2":"sanlingheyi2",
		"lingchuanxin2":"lingchuanxin2",
	]);
	int failed = 0;
	string links = "";
	string error_desc = "";
	...
	mixed err = catch {
		...
		if(player){
			...
			foreach(sort(indices(enhanced_books)), string book_name){
				string skill_name = enhanced_books[book_name];
...
void test_mystic_skills_runtime()
{
	...
	foreach(skill_names, string skill_name){
		...
		if(err || !skill || skill->skill_rare != "mystic" ||
		   search(skill->skill_type, "fangshi") == -1){
			failed++;
			werror("  ✗ %s: %s\n", skill_name,
				err ? describe_error(err) : "稀有度或职业标记错误");
		}
	}
	...
	if(failed == 0)
		...
		test_fail(sprintf("%d 个神秘技能运行时加载失败", failed));
}
