...
int main()
{
	...
	mixed err = catch{
		...
		check("首期稳定编号与展示名固定为S1",
			...
			(int)public_status["duration_days"]==90,
		...
		foreach(room_paths,string room_path){
			...
			if(room_err || !room || one_affinity=="")
				rooms_compile = 0;
			if(affinity=="") affinity = one_affinity;
			else if(one_affinity!=affinity) rooms_compile = 0;
		}
		check("S1地图全部可加载且始终归属同一个Worker一致性域",
			rooms_compile && affinity=="illusion_s1",
			"地图编译失败或affinity被拆分");
		...
		mapping duplicate = ACCOUNT_CHARACTERD->create_character(account_id,
			"human","jianxian","","","","illusion","S1");
		check("同一注册账号在S1只能创建一次且重试不生成第二档案",
			!(int)duplicate["ok"] &&
			search((string)duplicate["message"],"已经创建过")!=-1,
			sprintf("duplicate=%O",duplicate));
