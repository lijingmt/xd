...
void test_catalog_and_all_professions(array(object) players)
{
	...
	int valid_catalog = sizeof(catalog)==23 && sizeof(starters)==3 &&
		sizeof(bosses)==5;
	...
	check("22种公开异兽、1只隐藏鸾鸟、3只初契与5只轮替首领完整",
...
void test_hidden_luan_owner_revive()
{
	...
	check("鸾鸟累计70级以上真实首领且不限玩家等级差",
		chosen["ok"] && (int)before["catalog_total"]==22 &&
	...
	check("第500次合格首领无视随机值保底完整鸾鸟并重置账号计数",
		...
		(int)collected["catalog_total"]==23,
