...
assert(indexSource.includes('<span>🐾</span>共享宠物</button>'));
assert(indexSource.includes("sendQuickCommand('spirit_companion')"));
assert(indexSource.includes('<span>✦</span>本命灵伴</button>'));
...
assert(appSource.includes('/api/autofight_view?'));
assert(!appSource.includes("sendJsonCommand('flushview')"));
assert(appSource.includes('this.applyBattleStatusData(data.refresh, true)'));
assert(appSource.includes('标签隐藏/最小化只暂停渲染，不会暂停挂机'));
assert(/setInterval\s*\(\s*\(\)\s*=>\s*\{[\s\S]{0,120}runAutofightTick/.test(appSource));
